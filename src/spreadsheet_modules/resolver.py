...
class Resolver:
    ...
    def __init__(self):
        ...
        self.__mophic_df = self.__spreadsheet_reader("D:/Users/PC220321/Desktop/person_test.csv")
        ...
        print(self.__mophic_df.to_string())
    ...
    def merge_dataframes(self, hosxp_df: pd.DataFrame, mophic_df: pd.DataFrame) -> pd.DataFrame:
        dose1_df = hosxp_df.loc[~hosxp_df["dose_1"].isna(), ["cid", "vaccine_name_1", "dose_1", "date_1"]]
        dose2_df = hosxp_df.loc[~hosxp_df["dose_2"].isna(), ["cid", "vaccine_name_2", "dose_2", "date_2"]]
    ...
    def __rearrange_columns(self, mpd: pd.DataFrame) -> pd.DataFrame:
        return mpd[["name", "age", "cid", "cid_hash",
                    "vaccine_name_1", "dose_1", "date_1",
                    "vaccine_name_2", "dose_2", "date_2"
                    ]]

    def export_to_excel(self, path: str, merged_df: pd.DataFrame):
        merged_df.to_excel(path, sheet_name="Result")

    # Will Change to Both Dataframes check
    def __is_hosxp_df_empty(self) -> bool:
        if self.__hosxp_df is None or len(self.__hosxp_df) == 0:
            return True
        else:
            return False
